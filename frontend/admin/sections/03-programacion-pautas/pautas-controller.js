...
;(function (global) {
  ...
  // Prioridad: 1) window.Onix.getAudios()  2) library global de Biblioteca
  //            3) localStorage bajo clave conocida  4) array vacío con aviso
  // ──────────────────────────────────────────────────────────
  function _loadAudios() {
    // 1. API pública de Ónix (conectar aquí cuando esté disponible)
    if (typeof window.Onix?.getAudios === 'function') {
      const raw = window.Onix.getAudios();
      return _normalizeAudios(raw);
    }

    // 2. Variable global que expone la Sección 2 (Biblioteca Musical)
    if (Array.isArray(window.library) && window.library.length) {
      return window.library.map(a => ({
        id:       a.id,
        nombre:   a.archivo_path?.split('/').pop() || a.title || `Audio #${a.id}`,
        titulo:   a.title  || a.titulo  || '',
        artista:  a.artist || a.artista || '',
        duracion: _fmtDur(a.duration || 0),
      }));
    }

    // 3. localStorage — clave que usa la Biblioteca Musical
    try {
      const raw = localStorage.getItem('onix_library') || localStorage.getItem('onix_audios_v1');
      if (raw) return _normalizeAudios(JSON.parse(raw));
    } catch (_) { /* continuar */ }

    // 4. Sin fuente disponible — array vacío, el selector lo indica
    console.warn('[PautasController] No se encontró fuente de audios. ' +
      'Expón window.Onix.getAudios() desde la Biblioteca Musical.');
    return [];
  }
  ...
  function _selectAudio(audio) {
    const fAudio = $('fAudio');
    fAudio.value = audio.titulo
      ? `${audio.artista ? audio.artista + ' — ' : ''}${audio.titulo}`
      : audio.nombre;
    fAudio._selectedId     = audio.id;
    fAudio._selectedNombre = audio.nombre;
    $('audioHint').textContent = `✔ ${audio.nombre}${audio.duracion ? ' · ' + audio.duracion : ''}`;
    _hideEl($('audioDropdown'));
    // Limpiar búsqueda
    const si = $('audioSearchInput');
    if (si) si.value = '';
    _hideEl($('audioSearchClear'));
  }

  function _toggleAudioDropdown() {
    const dd = $('audioDropdown');
    if (dd.classList.contains('hidden')) {
      _audios = _loadAudios();          // recargar en cada apertura — datos frescos
      _renderAudioList();
      _showEl(dd);
   ...
   * Las que no pasen el predicado se apagan.
   */
  function _applyQuickFill(predicate) {
    document.querySelectorAll('.matrix-cell[data-key]').forEach((c, i) => {
      ...
      if (match) {
        c.classList.remove('matrix-cell--flash');
        // Stagger visual para efecto de "encendido"
  ...
  function _bindEvents() {
    ...
    $('audioSearchInput').addEventListener('input', e => {
      ...
      _renderAudioList(term);
      const clearBtn = $('audioSearchClear');
      if (clearBtn) term ? _showEl(clearBtn) : _hideEl(clearBtn);
    });
  ...
  function init() {
    _load();
    _audios = _loadAudios();   // ← datos dinámicos, no hardcodeados
    _bindEvents();
    _render();
    console.info('[PautasController] ✅ Módulo inicializado. Pautas:', _pautas.length,
      '| Audios disponibles:', _audios.length);
  }
